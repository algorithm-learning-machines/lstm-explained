...
\begin{align}
  ...
  \\
  \boldsymbol{\delta}_{\iota}^{(t)} &\overset{\text{not.}}{=} \dfrac{\partial E}{\partial \boldsymbol{w}_{\iota}^{(t)}} = \dfrac{\partial E}{\partial g_{\iota}^{(t)}} \dfrac{\partial g_{\iota}^{(t)}}{\partial \boldsymbol{w}_{\iota}^{(t)}} = \delta_{g_{\phi}}^{(t)} f_{\iota}'\left(a_{\iota}^{(t)}\right) \left[{\mathbf{x}^{(t)}}^{\text{T}}; {\mathbf{z}_{h}^{(t-1)}}^{\text{T}}; {\mathbf{s}^{(t-1)}}^{\text{T}}; 1\right]
\end{align}
